...
'''--- Server Information ---'''
##########
HOST = '192.168.137.188'  # The server's hostname or IP address
...
class Hands():
	...
	def showHold(self):
		...
		for i in range(len(self.cardHold)):
			##########
			card = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\card\\"+self.cardHold[i].showData()+".png")
			card.convert()
...
class Flag():
	...
	def showFlag(self):
		...
		if self.winner == 'A':
			##########
			flagGet = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\flags\\redflag.png")
			flagGet.convert()
			...
			##########
			flagGet = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\flags\\blueflag.png")
			flagGet.convert()
			bg.blit(flagGet,((self.flagID-1)*116,220))	#旗幟座標
			screen.blit(bg, (0,0))
			pg.display.update()
		...
		for i in self.containerA:
			##########
			card = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\card\\"+i.showData()+".png")
			card.convert()
...
class Board():
	...
	def show_allFlag(self):
		...
		# 背景: 棋盤
		##########
		imageBoard = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\遊戲介面_nofunc_forB.png")	#括弧為圖檔名稱		
		# 母牌堆
		##########
		imageBack = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\card\\Acardback.png")
		imageBoard.convert()
		imageBack.convert()
...
def movement_playerA():
	...
	#### server ####
	global board    
	board.all_flag[decision_A[1]].placeCard_inA(cardTaken)
...
# 開始視窗
##########
startbk = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\start.jpg")
startbk.convert()
bg.blit(startbk, (0,0))
screen.blit(bg, (0,0))
pg.display.update()
...
# 規則視窗
##########
rulebk = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\rule.jpg")
rulebk.convert()
bg.blit(rulebk, (0,0))
screen.blit(bg, (0,0))
pg.display.update()
...
next_step()
##########
rule2bk = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\rule2.jpg")
rule2bk.convert()
bg.blit(rule2bk, (0,0))
screen.blit(bg, (0,0))
pg.display.update()
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
	...
	for round in range(1,28):
		...
		if board.showWinner() != 0:
			##########
			endbk = pg.image.load("C:\\Users\\chen8\\Desktop\\BattleLine\\end.jpg")
			endbk.convert()
			bg.blit(endbk, (0,0))
			screen.blit(bg, (0,0)) 	
...
running = True
while running:
	event = pg.event.get()	#等待玩家點擊
	if event.type == pg.KEYDOWN and event.dict['key'] == QUIT:	#當空白鍵被按下則跳出
		running = False
pg.quit()
